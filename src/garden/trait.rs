use crate::garden::r#trait::TotalType::B;
use std::fmt::Debug;
...
fn main112() {
    ...
    // 把 Option 或 &Option 转换成 Option<&T>。创建一个新 Option，里面的类型是原来类型的引用，就是从 Option 到 Option<&T>。
    // 原来那个 Option 实例保持不变。
    ...
    // 创建一个新 Result，里面的两种类型分别是原来两种类型的引用，就是从 Result 到 Result<&T, &E>。原来那个 Result 实例保持不变
    let x: Result<u32, &str> = Ok(2);
    ...
    // 把 Option 的值拿出去，在原地留下一个 None 值。这个非常有用。相当于把值拿出来用，但是却没有消解原来那个 Option。
    let mut x = Some(2);
    let y = x.take();
    assert_eq!(x, None);
    assert_eq!(y, Some(2));
